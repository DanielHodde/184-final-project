...
from terrain.visualization.pyvista_vis import add_trees_to_plotter, plot_terrain
...
def generate_and_visualize_fractal_terrain():
    ...
    """
    terrain = generate_fractal_perlin_noise(
        shape=(2048, 2048),
        scale=50,
        octaves=6,
        persistence=0.5,
...
def generate_and_visualize_terrain_with_trees():
    ...
    tree_density = generate_fractal_perlin_noise(
        shape=terrain.shape, scale=15, octaves=3
    )
    tree_density = (tree_density - tree_density.min()) / (
        tree_density.max() - tree_density.min()
    )
    # Visualize terrain
...
def generate_and_visualize_single_tree_example():
    ...
    add_single_tree_to_plotter(
        plotter, terrain, tree_height=5, tree_radius=1, tree_color="red"
    )
    plotter.show()
