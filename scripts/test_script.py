...
def generate_terrain(size=128, octaves=6, persistence=0.5, lacunarity=2.0):
    ...
    for i in range(size):
        for j in range(size):
            nx = i/size - 0.5
            ny = j/size - 0.5
            terrain[i][j] = snoise2(nx, ny, octaves=octaves, 
...
def add_trees_with_variation(plotter, terrain, tree_indices, base_height=3, base_radius=0.7, color="forestgreen", seed=42):
    import numpy as np
    import pyvista as pv
    ...
    for idx in tree_indices:
        ...
        z = terrain[y, x]
        cone = pv.Cone(center=(x + dx, y + dy, z + height / 2), direction=(0, 0, 1), height=height, radius=radius, resolution=24)
...
def generate_tree_density(terrain, size=128):
    ...
    for i in range(size):
        for j in range(size):
            ...
            if height > min_height + 0.2 * (max_height - min_height) and height < min_height + 0.6 * (max_height - min_height):
                density[i, j] = 1.0 - abs(height - mid_height * 0.8) / (max_height * 0.4)
            else:
...
def visualize_terrain_with_trees(terrain, tree_density, tree_threshold=0.7):
    ...
    if len(tree_points) > 0:
        ...
        for pt, height, radius in zip(tree_points, tree_heights, tree_radii):
            cone = pv.Cone(center=(pt[0], pt[1], pt[2] + height/2), direction=(0, 0, 1), height=height, radius=radius, resolution=16)
